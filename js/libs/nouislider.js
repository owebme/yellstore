...
function closure ( target, options ){
	...
	function fireEvent ( event, handleNumber, start ) {
		...
		if (start === undefined){
			...
			var html = start.firstElementChild;
			if (html.className.match(/lower/)){
				html.innerHTML = String(options.start[0]).replace(/(\d)(?=((\d{3})+)(\D|$))/, "$1 ");
			}
			else if (html.className.match(/upper/)){
				html.innerHTML = String(options.start[1]).replace(/(\d)(?=((\d{3})+)(\D|$))/, "$1 ");
			}
		}
	}
	...
	function setHandle ( handle, to, noLimitOption ) {
		...
		if ((options.spectrum.xVal[1]/options.spectrum.xVal[0] > 11 && options.singleStep > 0.99)){
		
			var value;
			
			if (trigger == "0"){
				value = scope_Spectrum.fromStepping( to * (1.2 + options.margin/100) );
			}
			else {		
				value = scope_Spectrum.fromStepping( to );
				
				if (to < 80 && to > 50){
					value = value * ((to/100) * 1.2);
				}
				else if (to < 51){
					value = value * ((to/100) * 0.9);
				}
				else if (to < 41){
					value = value * ((to/100) * 0.64);
				}
			}

			if (options.singleStep > 0 && value % options.singleStep != 0) {value = parseInt(value / options.singleStep) * options.singleStep;}
			
			scope_Values[trigger] = value;
		}
		else {
	...
	function valueGet ( ) {
		...
		for ( i = 0; i < options.handles; i += 1 ){
			...
			if (options.singleStep < 1) value = options.format.to(value);
			retour[i] = String(value).replace(/(\d)(?=((\d{3})+)(\D|$))/, "$1 ");
		}
